...
// Tinted chips for the dark shell. These were pastel -100 fills with black
// text, which glared against everything around them.
const CATEGORY_COLORS: Record<string, string> = {
  economy: "bg-amber-400/15 text-amber-300",
  xp: "bg-sky-400/15 text-sky-300",
  daily: "bg-emerald-400/15 text-emerald-300",
  streak: "bg-orange-400/15 text-orange-300",
  level: "bg-violet-400/15 text-violet-300",
  config: "bg-slate-400/15 text-slate-300",
  inventory: "bg-pink-400/15 text-pink-300",
  admin: "bg-red-400/15 text-red-300",
  quests: "bg-teal-400/15 text-teal-300",
};
  ...
  return (
    <span
      className={`rounded-md px-1.5 py-0.5 text-xs font-semibold ${
        value >= 0 ? "bg-emerald-400/10 text-emerald-300" : "bg-red-400/10 text-red-300"
      }`}
    >
      {label} {value >= 0 ? "+" : ""}{value}
    </span>
...
function LogRow({ log }: { log: LogEvent }) {
  ...
  const categoryColor = CATEGORY_COLORS[log.category] ?? "bg-slate-400/15 text-slate-300";
  ...
  return (
    ...
    <div className="flex items-center gap-1 text-xs text-[var(--muted)]">
      <span>
        ...
        <span className="font-mono text-[var(--foreground)]">{log.user_id}</span>
        )}
      </span>
      {log.target_user_id != null && (
        ...
        {log.metadata?.targetDiscordId ? (
          ...
          <span className="font-mono text-[var(--foreground)]">{log.target_user_id}</span>
        )}
        </span>
      )}
    </div>
      ...
      {log.item_id && (
        <div className="text-xs text-[var(--muted)]">
          Item: <span className="font-mono text-[var(--foreground)]">{log.item_id}</span>
