...
# package.json present, even though only havocish is built here.
COPY package.json package-lock.json ./
COPY havocish/package.json ./havocish/
...
COPY tsconfig.json ./
COPY havocish ./havocish

RUN npm --workspace havocish run build
...
COPY --from=build /app/havocish/dist ./havocish/dist
