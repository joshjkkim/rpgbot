...
export async function addMessageXp(args: XpArgs): Promise<{profile: DbUserGuildProfile, gave: boolean, levelUp: boolean}> {
    ...
    if (hasDiscordCtx && hasAnyAchievementEffects) {
        await applyAchievementSideEffects({
            ...
            config,
            unlocked: achievementResults.unlocked,
            rewards: achievementResults.rewards
            ? {
                grantedRoles: achievementResults.rewards.grantedRoles,
                messages: achievementResults.rewards.messages,
                }
            : null,
        });
    }
...
export async function grantDailyXp(args: XpArgs): Promise<{profile: DbUserGuildProfile, granted: boolean, rewardXp?: number, rewardGold?: number, levelUp: boolean, 
    ...
    if (hasDiscordCtx && hasAnyAchievementEffects) {
        await applyAchievementSideEffects({
            ...
            member: args.member ?? null,
            channelIdHint: args.channelId ?? null, // if your XpArgs includes it
            config,
            unlocked: achievementResults.unlocked,
            rewards: achievementResults.rewards
            ? {
